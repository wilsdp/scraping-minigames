...
const { insertBl2Data } = require("../models/bl2Model")
...
const bosLadder2 = {
  start: async () => {
    ...
    await bosLadder2.createGame(now.minute())
    bosLadder2.showGame()
    intervalID = setInterval(bosLadder2.main, 1000)
  },
  ...
  main: () => {
    ...
    if ((now.minute() + 1) % interval_2 === 0 && now.second() === 58) {
      bosLadder2.getResult()
    }
    if (now.minute() % interval_2 === 0 && now.second() === 5) {
      bosLadder2.showResult()
      bosLadder2.createGame(now.minute() + 1)
      bosLadder2.showGame()
    }
  },
  ...
  createGame: async (min) => {
    try {
      const now = getCurrentTimeInSeoul()
      const round =
        bosLadder2.getRound(now.hour() * 3600 + min * 60 + now.second()) /
        (interval_2 * 60)
      ...
      if (!previousGames.some((g) => g.round === round)) {
        const minutesUntilNextInterval =
          (interval_2 - ((now.minute() + 1) % interval_2)) % interval_2
      ...
      const responseModel = await insertBl2Data(game, false)
  ...
  getResult: async (retryCount = 0) => {
    try {
      const options = {
        ...
        headers: {
          cookie:
            "AWSALB=lgjLjyY%2FbK0nP3JqcFUlFpemJ7lSuoypnI0ZN2UzxfACiNL3q161%2BQh4zfdz6UBU6Y3bDzozfmtTji5S4tBvsj8%2FgS2C5a8rly%2BN0T28Ovp5LliVvVAy2Llvws2m; AWSALBCORS=lgjLjyY%2FbK0nP3JqcFUlFpemJ7lSuoypnI0ZN2UzxfACiNL3q161%2BQh4zfdz6UBU6Y3bDzozfmtTji5S4tBvsj8%2FgS2C5a8rly%2BN0T28Ovp5LliVvVAy2Llvws2m; PHPSESSID=qr3bos6fp4p5g2k0n9oh9per7l; 2a0d2363701f23f8a75028924a3af643=MjIyLjEyNy4xODkuMjQ5",
          "Content-Type":
      ...
      const responseModel = await insertBl2Data(game, true)
      if (!responseModel.success) {
        throw new Error(responseModel.error)
      } else {
        console.log(
      ...
      if (retryCount < maxRetries) {
        ...
        return bosLadder2.getResult(retryCount + 1) // Recursive retry
      } else {
        console.error(
  ...
  showResult: async () => {
    ...
    await bosLadder2.delay(10000)
  },
}
module.exports = bosLadder2
