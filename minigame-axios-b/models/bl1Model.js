...
const BL1Model = sequelize.define(
  "bl1Data",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "BOS_LADDER1M",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertBl1Data = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await BL1Model.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await BL1Model.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await BL1Model.create(payload)
