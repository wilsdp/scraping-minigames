...
const BL3Model = sequelize.define(
  "bl3Data",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "BOS_LADDER3M",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertBl3Data = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await BL3Model.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await BL3Model.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await BL3Model.create(payload)
