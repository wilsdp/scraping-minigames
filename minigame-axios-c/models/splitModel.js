...
const gstSplitModel = sequelize.define(
  "splitData",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "GST_SPLIT30S",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertSplitData = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await gstSplitModel.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await gstSplitModel.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await gstSplitModel.create(payload)
