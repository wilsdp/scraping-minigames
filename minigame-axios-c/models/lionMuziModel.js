...
const gstLMModel = sequelize.define(
  "ladderData",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "GST_LIONMUZI30S",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertGSLM30sData = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await gstLMModel.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await gstLMModel.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await gstLMModel.create(payload)
