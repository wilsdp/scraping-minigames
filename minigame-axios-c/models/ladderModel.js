...
const gstLadderModel = sequelize.define(
  "ladderData",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "GST_LADDER30S",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertLadderData = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await gstLadderModel.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await gstLadderModel.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await gstLadderModel.create(payload)
