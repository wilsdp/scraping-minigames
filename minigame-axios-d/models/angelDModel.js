...
const jwcADModel = sequelize.define(
  "jwcADData",
  {
    mini_code: {
      type: DataTypes.STRING(24),
      defaultValue: "JWC_ANGELDEMON",
      primaryKey: true,
      allowNull: false,
    },
    mini_rotation_id: {
...
exports.insertADData = async (game, isUpdate = false) => {
  const payload = {
  ...
  try {
    ...
    const existingRecord = await jwcADModel.findOne({
      where: {
        mini_code: game.gameName,
        mini_rotation_id: game.rotation,
      },
    ...
    if (existingRecord && isUpdate) {
      ...
      dataResult = await jwcADModel.update(payload, {
        where: {
          mini_code: game.gameName,
          mini_rotation_id: game.rotation,
        },
      ...
      dataResult = await jwcADModel.create(payload)
